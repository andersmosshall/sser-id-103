...
confirm_ssr_id=""; while [[ "$confirm_ssr_id" != "y" && "$confirm_ssr_id" != "n" ]]; do read -p "   Is the extracted SSR ID '$ssr_id' correct? [Y/n]: " confirm_ssr_id_input; confirm_ssr_id=${confirm_ssr_id_input:-y}; confirm_ssr_id=$(echo "$confirm_ssr_id" | tr '[:upper:]' '[:lower:]'); done
...
echo ""; echo "   ================ Manual Step 2: Drupal Web Installation ================"; echo "   ACTION REQUIRED: Visit site URL. Use: Lang=En, Profile=Minimal, Prefix='ss${ssr_id}_', Site='$school_name', Emails='$noreply_email'/'$bug_email', User1Pass=STRONG"; echo "   (Script does not do this)"; echo "   ======================================================================"; echo ""; echo "------------------------------------------------------------"
