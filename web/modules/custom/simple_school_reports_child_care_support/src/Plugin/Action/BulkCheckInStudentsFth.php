 ...
 *
 * @Action(
 *   id = "ssr_bulk_check_in_students",
 *   label = @Translation("Bulk check in students (fth)"),
 *   type = "user",
 *   confirm_form_route_name = "simple_school_reports_child_care_fth_check_in.check_in_students"
 * )
 */
class BulkCheckInStudentsFth extends ActionBase implements ContainerFactoryPluginInterface {
  ...
  public function executeMultiple(array $entities) {
    ...
    $this->tempStoreFactory->get('child_care_check_in_students')->set($this->currentUser->id(), $ids);

    $date = $this->childCareService->getDateFromRequest();
    $this->tempStoreFactory->get('child_care_check_in_date')->set($this->currentUser->id(), $date?->format('Y-m-d'));
  }
  ...
  public function access($object, AccountInterface $account = NULL, $return_as_object = FALSE) {
    ...
    $student_access = AccessResult::allowedIf($date && $this->childCareCheckInService->allowStudentCheckIn($user->id(), $date))
