...
/**
 * Provides a WrittenReviewsRoundProgress block.
 *
 * @Block(
 *  id = "written_reviews_round_progress_block",
 *  admin_label = @Translation("Written reviews round progress block"),
 * )
 */
class WrittenReviewsRoundProgressBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    $build['label'] = [
      '#type' => 'html_tag',
      '#tag' => 'h3',
      '#value' => $this->t('Register written reviews for @name', ['@name' => $node->label()]),
    ];
    $build['progress'] = [
      '#theme' => 'progress_bar',
      '#label' => $this->t('Written reviews round progress'),
      '#percent' => $value,
    ];
  ...
  public function blockAccess(AccountInterface $account) {
    ...
    if ($node = $this->routeMatch->getParameter('node')) {
      ...
      if ($node instanceof NodeInterface && $node->bundle() === 'written_reviews_round') {
