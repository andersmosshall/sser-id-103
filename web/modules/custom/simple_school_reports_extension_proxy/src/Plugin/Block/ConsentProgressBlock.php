...
/**
 * Provides a ConsentProgressBlock block.
 *
 * @Block(
 *  id = "consent_progres_block",
 *  admin_label = @Translation("Consent progress block"),
 * )
 */
class ConsentProgressBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    $cache->addCacheTags(['ssr_consent_answer_list:' . $node->id()]);
    ...
    $value = $consent_service->getConsentCompletion($node->id());


    $build['progress'] = [
      '#theme' => 'progress_bar',
      '#label' => $this->t('Consent progress'),
      '#percent' => $value,
    ];
