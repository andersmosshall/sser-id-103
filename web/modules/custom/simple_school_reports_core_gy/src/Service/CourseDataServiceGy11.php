...
/**
 * Course data service for Gymnasiet (2011).
 */
class CourseDataServiceGy11 implements CourseDataServiceGyInterface {
  ...
  public function getCourseData(): array {
    $cid = 'course_data_gy11';
    if (isset($this->lookup[$cid])) {
      return $this->lookup[$cid];
    }

    $path = $this->moduleHandler->getModuleDirectories()['simple_school_reports_core_gy'] . DIRECTORY_SEPARATOR . 'files' . DIRECTORY_SEPARATOR . 'course_catalog_gy11.csv';
    if (!file_exists($path)) {
      return [];
    }
    ...
    while (($row = fgetcsv($handle)) !== FALSE) {
      ...
      if ($first_row) {
        ...
        if (count($row) < 5 || $row[0] !== 'Course' || $row[1] !== 'Course code' || $row[2] !== 'Subject' || $row[3] !== 'Subject code' || $row[4] !== 'Link' || $row[5] !== 'Points') {
          break;
        }
        continue;
      }
      ...
      $course_data[$course_code] = [
        ...
        'link' => $subject_link,
        'use_langcode' => str_contains($course_code, 'XXX'),
        'official' => TRUE,
        'grade_vid' => 'af_grade_system',
        'group_for' => [],
        'levels' => [],
        'points' => (int) $points,
        'school_type_versioned' => 'GY:2011',
      ];
    }
    fclose($handle);
  ...
  public function getSubjectsData(): array {
    ...
    $tids = $this->entityTypeManager->getStorage('taxonomy_term')->getQuery()
      ...
      ->condition('field_school_type_versioned', ['GY:2011'], 'IN')
